...
class TrackerLogger {
    ...
    private lastPrintedLines: number = 0;
    ...
    debug(...args: any[]) {
        dashboard.log(args.map(a => String(a)).join(' '));
    }
    ...
    private clearLastBox() {}

    private generateGraph(history: number[], threshold: number, height: number = 10, width: number = 54): string[] {
        return [];
    }

    formatDeviceState(jid: string, rtt: number, avgRtt: number, median: number, threshold: number, state: string, history: number[] = []) {
        dashboard.updateStats(jid, state, rtt, avgRtt, median, threshold);
    }
}
...
interface DeviceMetrics {
    ...
    recentRtts: number[];      // Recent RTTs for moving average (last 5)
    state: string;             // Current device state (Online/Standby/Calibrating/Offline)
    lastRtt: number;           // Most recent RTT measurement
    lastUpdate: number;        // Timestamp of last update
    presence?: string;         // Explicit presence status ('available', 'composing', etc.)
}
...
export class WhatsAppTracker {
    ...
    private probeInterval: number = 3000; // Default to 3 seconds
    ...
    public setProbeMethod(method: ProbeMethod) {
        ...
        trackerLogger.info(`\n[INFO] Probe method changed to: ${method === 'delete' ? 'Silent Delete' : 'Reaction'}\n`);
    }
    ...
    public setProbeInterval(interval: number) {
        this.probeInterval = Math.max(1000, Math.min(interval, 60000));
        trackerLogger.info(`\n[INFO] Probe interval changed to: ${this.probeInterval}ms\n`);
    }
    ...
    public async startTracking() {
        ...
        trackerLogger.info(`\n[INFO] Tracking started for ${this.targetJid}`);
            ...
            if (update.presences) {
                for (const [jid, presenceData] of Object.entries(update.presences)) {
                    if (presenceData && presenceData.lastKnownPresence) {
                        ...
                        // Store this presence update per-device if possible
                        if (this.deviceMetrics.has(jid)) {
                            this.deviceMetrics.get(jid)!.presence = presenceData.lastKnownPresence;
                        }

                        this.lastPresence = presenceData.lastKnownPresence;
                        trackerLogger.debug(`[PRESENCE] Stored presence from ${jid}: ${this.lastPresence}`);
                        
                        // Force a state re-determination for this JID
    ...
    private async probeLoop() {
        while (this.isTracking) {
            ...
            const delay = this.probeInterval + Math.floor(Math.random() * 500); // Add jitter
    ...
    private markDeviceOffline(jid: string, timeout: number) {
        ...
        trackerLogger.info(`\n[OFFLINE] Device ${jid} marked as OFFLINE (no CLIENT ACK after ${timeout}ms)\n`);
        this.sendUpdate();
    }
    ...
    private addMeasurementForDevice(jid: string, rtt: number) {
        ...
        if (rtt <= 5000) {
            // 1. Add to device's recent RTTs for moving average (last 5)
            metrics.recentRtts.push(rtt);
            if (metrics.recentRtts.length > 5) {
    ...
    private determineDeviceState(jid: string) {
        ...
        // 1. Prioritize official WhatsApp presence if available
        // 'available', 'composing', 'recording' all indicate active usage
        const isOfficiallyOnline = metrics.presence === 'available' || 
                                   metrics.presence === 'composing' || 
                                   metrics.presence === 'recording' ||
                                   this.lastPresence === 'composing' ||
                                   this.lastPresence === 'recording';

        if (isOfficiallyOnline) {
            metrics.state = 'Online';
            trackerLogger.debug(`[DEVICE ${jid}] Marked Online due to explicit presence: ${metrics.presence || this.lastPresence}`);
            this.sendUpdate();
            return;
        }

        // 2. Fall back to RTT analysis
        ...
        // Need at least 10 samples for a stable baseline
        if (this.globalRttHistory.length >= 10) {
            const sorted = [...this.globalRttHistory].sort((a, b) => a - b);
            const mid = Math.floor(sorted.length / 2);
            median = sorted.length % 2 !== 0 ? sorted[mid] : (sorted[mid - 1] + sorted[mid]) / 2;

            // 0.95 is more permissive than 0.9, helping detect "online" on faster/jittery networks
            threshold = median * 0.95;
        ...
        trackerLogger.formatDeviceState(jid, metrics.lastRtt, movingAvg, median, threshold, metrics.state, metrics.rttHistory);
    ...
    private sendUpdate() {
        ...
        const globalThreshold = globalMedian * 0.95;
    ...
    private calculateGlobalMedian(): number {
        if (this.globalRttHistory.length < 10) return 0;
