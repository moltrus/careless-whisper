...
if (debugMode) {
    originalConsoleLog('[DEBUG] Debug mode enabled\n');
} else {
    originalConsoleLog('[INFO] Normal mode (important outputs only)\n');
...
async function connectToWhatsApp() {
    ...
    originalConsoleLog('[INFO] Connecting to WhatsApp... (use the --debug flag for more details)');
    ...
    sock.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'close') {
            ...
            originalConsoleLog('[INFO] Connected to WhatsApp');
            isConnected = true;

            if (currentTargetJid) {
                if (debugMode) {
                    originalConsoleLog(`[INFO] Resuming tracking for ${currentTargetJid}...`);
                }
                dashboard.init(); // Init if we are resuming immediately
        ...
        rl.question('Enter target phone number (with country code, e.g., 491701234567): ', async (number) => {
            ...
            try {
                ...
                if (result?.exists) {
                    ...
                    currentTracker.startTracking();
                    dashboard.log(`Tracking started for ${result.jid}`);
                    rl.close();
                } else {
                    originalConsoleLog('[ERROR] Number not registered on WhatsApp.');
                    rl.close();
                    askForTarget();
                }
            } catch (err) {
