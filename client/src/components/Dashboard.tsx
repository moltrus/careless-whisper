...
export function Dashboard({ connectionState }: DashboardProps) {
    ...
    const [rateLimit, setRateLimit] = useState(0.33); // Default ~0.33 requests/sec (3000ms)
    ...
    useEffect(() => {
        ...
        function onTrackedContacts(contacts: { id: string }[]) {
            setContacts(prev => {
                const next = new Map(prev);
                contacts.forEach(({ id }) => {
                    if (!next.has(id)) {
        ...
        function onRateLimitUpdated(rate: number) {
            setRateLimit(rate);
        }
        ...
        socket.on('rate-limit-updated', onRateLimitUpdated);
        ...
        return () => {
            ...
            socket.off('rate-limit-updated', onRateLimitUpdated);
    ...
    const handleAdd = () => {
        ...
        socket.emit('add-contact', { number: inputNumber, platform: 'whatsapp' });
    };
    ...
    const handleRateLimitChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        setRateLimit(val);
        socket.emit('set-rate-limit', val);
    };
                ...
                <div className="flex items-center gap-4 mb-4 p-4 bg-gray-50 rounded-lg">
                    <div className="flex-1">
                        <div className="flex justify-between mb-2">
                            <label className="text-sm font-medium text-gray-700">Probe Rate (Requests/Second)</label>
                            <span className="text-sm font-bold text-blue-600">{rateLimit.toFixed(1)} req/s</span>
                        </div>
                        <input
                            type="range"
                            min="0.1"
                            ...
                            step="0.1"
                            value={rateLimit}
                            onChange={(e) => handleRateLimitChange(e)}
                            className="w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer"
                        />
                        <div className="flex justify-between text-xs text-gray-500 mt-1">
                            <span>Slow (0.1/s)</span>
                            <span>Mid (2.5/s)</span>
